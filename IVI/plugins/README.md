...
### 2. 系统 `kzjava.jar`：Studio `EnginePlugins`（不要放进仓库 `IVI/plugins/`）
...
### 3. 业务 Java 插件：仓库 `IVI/plugins/`（3.9.5+ 路径规范）

自研 JAR 放在：

```
IVI/plugins/datasource/lib/java/Release/DroidDataSourceplugin.jar
```

（若有 Debug 包，也可放 `lib/java/Debug/`。）

Studio 导入路径：`IVI/plugins/datasource/lib/java/Release/DroidDataSourceplugin.jar`
...
| Preview Working Directory | `..\..\..\assets`（相对 `Tool_project/`） | 同左 |
...
**不要**把 `kzjava.jar` / `kzjvm.dll` 放进 `IVI/plugins/`。
