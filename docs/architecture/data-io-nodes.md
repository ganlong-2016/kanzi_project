...
> 清单产物 2/9。定义本工程里 **UI 节点如何"收数据"与"发数据"** 的标准做法:有哪几种节点/机制、分别何时用、每条链路的完整路径,并给出与 `assets/datasource.xml` 的字段对照。
...
- 每个"接收/发送"链路必须能落到 `assets/datasource.xml` 的一个字段;**没有字段就先改契约**(评审),再连线。
