...
数据字段定义见 [`IVI/assets/xml/datasource.xml`](../IVI/assets/xml/datasource.xml) 的 `Demo` 分组。
...
2. `launcher`:导入插件 → 数据源指向 `IVI/assets/xml/datasource.xml` → Screen 设 Data Context。
