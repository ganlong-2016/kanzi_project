...
**本仓库状态**: `IVI/demo/demo.kzproj` 已含 `Card` / `LabelText` / `ToggleSwitch` / `Slider`;`IVI/common/common.kzproj` 的 `Prefabs` 下仅剩 Studio 内置预览项。若你本地仍是旧布局,按下面步骤在 **Kanzi Studio 3.9.15** 迁移一次。
...
1. 打开 `IVI/demo/demo.kzproj`
2. **File → Import → Merge Project** → 选 `IVI/common/common.kzproj`
